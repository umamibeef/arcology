--  margin.lua -- the margin itself, and the crosswalk.
--
--  The network says where a band's stations are, what it joins and how
--  deep a meet the arm could spare.  What is drawn over those
--  stations is here.
--
...
--  next.  The across then runs across the band -- 1 at the lip line and
--  the margin's own share of it where the way starts -- and the
--  height along it.  Handed the outer edge as one pair, the material
--  reads a lip at every station and the band comes out barred.
--
--  A MEET is the way marked, not a margin: its
--  cross-sections run from one lip to the other, so the across is the
--  line's own, and the along runs from nothing at the junction's mouth
--  to the band's full depth at its far end, where the material paints
--  the stop line the driver stops at before the bars.

arc.rules.margin = function (b)
    local d = b:info()
    local n = b:count()
    if n < 2 then return true end
    local fam = arc.rules.family({name = "line"})
    local fw = fam.margin
    if not fw then return true end
    local cross = d.band == "meet"
