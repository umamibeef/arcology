...
--
--  A strip does not follow the ground.  It SPURS between the nodes at
--  its two ends, and through any level meet on the way.
--
--  A node -- a junction, a dead end, a level meet -- stands at its
--  own tile's levelled height, and every corridor that reaches it spurs
--  to that one number.  Two segments meeting at a junction therefore
--  agree without anything being solved between them; a line and a
--  railway meet agree because the meet is a node they share;
--  and an edit moves only the segments whose anchors moved.
--
--  An end the strip does not reach a node for keeps the height the
--  ground gave it, and the spur runs from there.
--
...
--  the shelf there as the box's own threads do.  Pinned to its node
--  instead it spurs a level down into the hillside it crosses and draws
--  itself under the ground.  A dead end is pinned whatever the family,
--  because it ends on its own tile.
--
--  The spur is EASED at both ends, so a corridor leaves a node level and
--  picks up its grade in between rather than kinking at the join.

local f32 = arc.put.f32

--  Two anchors closer together than this are one anchor: a meet a
--  hair from the end it follows would make a step, not a spur.
local APART = 0.5

--  As many anchors as a strip may have.  A strip with more level
--  meets than this on it takes the first of them.
local MAX = 64
...
arc.rules.ground = function (g)
    ...
    --  Every level meet along the way is a node of both networks.
    for i = 1, n - 2 do
        if na < MAX then
            local z = g:lap(i)
