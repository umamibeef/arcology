--  deck.lua -- what a freeway deck carries beside its carriageway.
--
--  A deck is a slab on columns, nothing under or beside it: the side
--  faces show through the carriageway from the far edge.  Two things are
--  built with each of its quads, and the strip's own rule calls them --
--  the gore before the quad, the underside after it.
--
--  THE GORE (spec 7.3): where the deck's outer lane parts from the
--  carriageway, the lane's own slab beside the narrowing deck, level,
--  from the slab's edge out to the deck's -- a sliver widening to a
--  lane.  The slab alone; the ramp itself is a piece of its own from
--  here to the road.  Laid BEFORE the deck's own slab: at the game's
...
--  arc.geo.deck_underside until they are a pass of their own.

local f32 = arc.put.f32

--  Concrete, the piers' material: it carries a depth bias of its own so
--  the structure stays behind the deck it holds up.
...
function arc.deck.gore(s, d, i, pv, cu, ha, hb, order, ala, alb)
    ...
    for sd = 0, 1 do
        ...
        if (pl | cl) & bit ~= 0 and 1.0 - 0.5 * (wa + wb) >= 0.01 then
            ...
            s:quad(o0x, o0y, i0x, i0y, o1x, o1y, i1x, i1y, za, zb,
                   ...
                   ala + GORE_ALONG, alb + GORE_ALONG, arc.mat.hiway, order)
        end
    end
end

function arc.deck.under(s, d, i, n, pv, cu, a0x, a0y, a1x, a1y, b0x, b0y, b1x, b1y, order)
    if d.flat or arc.geo.deck_underside < 0.5 then return end
    --  Behind the deck in the painter's order: depth here is the sweep's
    --  slot, not a height, so a soffit at the same order as the road it
    ...
    s:tri_n(a0x, a0y, u0z, b0x, b0y, v0z, b1x, b1y, v0z, 0, 0, -1,
            CONC[1], CONC[2], CONC[3], order)
    --  The ends: where the deck begins and ends in the air, a wall
    --  across it from the road down to the soffit.
    ...
    --  The girder's fascia down from the carriageway and the parapet up
