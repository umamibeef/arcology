--  walks.lua -- the margin beside a strip.
--
--  Two bands, one each hand of the way, station for station with
--  it: the outer edge at the way's own full width and the inner
--  one where the margin's share of the band begins.  That inner line is
--  where the way stops, and both are laid by arc.band_edge, so
...
--  margin is one walk with the junction's corner at each end, and that
--  is the network's bookkeeping rather than a shape.

arc.rules.walks = function (s)
    local d = s:info()
    if not d.lips then return false end
    local fam = arc.rules.family({name = d.family, width = d.half * 2})
    if not fam.margin then return false end
    local n = s:count()
    ...
    for side = 0, 1 do
        ...
        for k = 0, n - 1 do
            ...
            local ix, iy = arc.band_edge(x, y, dx, dy,
                                         arc.band_half(d.half, wf, fam.margin.inner), w, sgn)
            s:walk_at(side, ox, oy, ix, iy, z)
        end
        --  The band's two ends reach the strip's own, and stand out at
        --  the margin's outer edge rather than the way's: that
        ...
        local h0 = arc.band_half(d.half, s:width(dx0, dy0), fam.margin.edge)
        local h1 = arc.band_half(d.half, s:width(dx1, dy1), fam.margin.edge)
