...
typedef struct
{
     ...
     *  statics in app.c rather than travelling with the rest. */
    ...
    int         outline;                /* --outline: the roads stand aside for the curves and the footway network */
    char        assets_dir[1024], city_path[1024];
} Startup;

/*  Returns 0 to carry on, -1 when the arguments asked for something that
 *  is over already and well (--help), or an exit code (a bad flag, said
 *  already). */
#ifdef __cplusplus
extern "C"
#endif
