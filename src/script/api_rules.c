 ...
 *  decision over; leaving it unset, or answering nil, leaves the C's own
 *  answer standing, which is the only reason there is one path here and
 *  not two.  Every rule is called while the mesh is being built, a few
 *  thousand times a build, never once a frame.
 *
 ...
 *
 *      arc.rules.crossing_at(mouth) -> depth in tiles, or 0
 *          Whether one arm's mouth carries a crosswalk and how deep a
 *          band it asks for.  `mouth` carries col, row, arm, control,
 *          pavement (a footway each side of the mouth), cos (how the two
 *          run against one another, -1 squarely facing) and span (how
 *          wide the mouth is, in footway widths).
 *
 *      arc.rules.crossing(mouth) -> depth in tiles, or false
 *          Whether one arm's mouth carries a crosswalk and how deep its
 *          band runs.  `mouth` carries col, row, arm, control, want (the
 *          band the C would give it), room (the road it may give up) and
 *          straight (how far that road runs straight from the mouth).
 *
 ...
 *          a kerb return, false leaves it square, nothing runs the
 *          boundary straight past it.  `at` also carries col, row, grow
 *          (how far the junction is let out), width (its footway's) and,
