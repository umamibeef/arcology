...
typedef struct
{
    ...
    double   pos, step;
    float    amp, env;
    int      hold; /* frames left at full volume after the note-off, then the cut */
    unsigned age;
} Voice;
...
struct RMusic
{
    ...
    int      chan_prog[16];
    unsigned age;
    /* what the mix did, for the log: notes started, stolen, and dropped for want of an instrument */
    int n_started, n_stolen, n_dropped;
...
static int load_json(RMusic *m)
{
    ...
    if (!f)
        return -1;
    /*  A file that will not measure has no length to read, and a
     *  negative one read as a size is the whole address space. */
...
static int load_midi(RMusic *m, const char *path)
{
    ...
    free(raw);
    {
        /*  The voices this song gets.  The SONG resource's maxNotes is a
         *  flat 6 on every song, but their own MIDI wants anywhere from 4
        ...
        {
            unsigned char hi = m->ev[ev].st & 0xF0;
            if (hi == 0x90 && m->ev[ev].b)
                cur++;
            else if (hi == 0x80 || (hi == 0x90 && !m->ev[ev].b))
                cur -= cur > 0;
            if (cur > peak)
                peak = cur;
        }
        int want = pin ? atoi(pin) : peak;
        if (want < m->cur.max_notes)
...
{
    int c;
    memset(m->v, 0, (size_t)m->n_voices * sizeof *m->v);
    for (c = 0; c < 16; c++)
    {
        m->chan_vol[c]  = 1.0f;
...
static Voice *find_voice(RMusic *m)
{
    Voice *v = NULL;
    int    k;
    for (k = 0; k < m->n_voices; k++)
        if (!m->v[k].on)
            return &m->v[k];
    for (k = 0; k < m->n_voices; k++)
        if (m->v[k].releasing && (!v || m->v[k].amp * m->v[k].env < v->amp * v->env))
            v = &m->v[k];
    if (v)
        return v;
    if (m->grow)
    {
        Voice *nv = (Voice *)realloc(m->v, (size_t)(m->n_voices + 1) * sizeof *nv);
        if (nv)
        {
            m->v = nv;
            k    = m->n_voices++;
            memset(&m->v[k], 0, sizeof m->v[k]);
            return &m->v[k];
        }
        /* out of memory: fall through and steal the quietest */
    }
    for (k = 0; k < m->n_voices; k++)
        if (m->v[k].on && (!v || m->v[k].amp * m->v[k].env < v->amp * v->env))
            v = &m->v[k];
    return v;
}
...
static void note_on(RMusic *m, int chan, int note, int vel)
{
    ...
    float       amp  = ((float)vel / 127.0f) * m->chan_vol[chan];
    Voice      *v;
    int         base;
    if (!smp)
    {
...
static void note_off(RMusic *m, int chan, int note)
{
    ...
    int    k;
    for (k = 0; k < m->n_voices; k++)
        if (m->v[k].on && !m->v[k].releasing && m->v[k].chan == chan && m->v[k].note == note && (!v || m->v[k].age > v->age))
            v = &m->v[k];
    if (!v)
        return;
    v->releasing = 1;
    v->hold      = RELEASE_SLICES * SLICE_FRAMES;
}
    ...
    switch (e->st & 0xF0)
    {
        ...
        case 0xB0:
            ...
            else if (e->a == 123 || e->a == 120)
            {
                int k;
                for (k = 0; k < m->n_voices; k++)
                    if (m->v[k].on && m->v[k].chan == chan && !m->v[k].releasing)
                    {
                        m->v[k].releasing = 1;
                        m->v[k].hold      = RELEASE_SLICES * SLICE_FRAMES;
                    }
            }
            break;
        case 0xC0:
...
static void render(RMusic *m, float *out, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        float mix  = 0.0f;
        int   live = 0;
        /*  The events and their count come together: a song with none
         *  has no list to walk. */
        while (m->ev && m->next_ev < m->n_ev && m->ev[m->next_ev].t <= m->t)
...
static int start(RMusic *m, const Song *s)
{
    ...
    m->cur_id = s->id;
    song_reset(m);
    R_NOTE("music", "%s (%d), %.0f s", s->name, s->id, s->seconds);
    return 0;
}
...
void music_set_enabled(RMusic *m, int on)
{
    ...
    else
    {
        ...
        music_update(m);  /* and this one may as well */
    }
}
...
int music_render_wav(RMusic *m, const char *song, const char *path, int rate)
{
    ...
    fclose(f);
    R_DBG("music", "%s: %d notes, %d voices, %d stolen, %d no-instrument", m->cur.name, m->n_started, m->n_voices, m->n_stolen, m->n_dropped);
    m->cur_id = 0;
    return 0;
}
