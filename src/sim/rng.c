 ...
 *  library rand().  State is a plain 32-bit LCG at A5-0x3EDC with the ANSI
 *  constants, and the result is the HIGH word masked to 15 bits, taken
 *  modulo the argument by a divu.  It is NOT the LFSR at $20F30 and NOT the
 *  Toolbox _Random.  Using game_rand() for it in pickDirection ($E19A) drew
 *  from the wrong stream and put the whole cycle out of step from draw 1411
 *  on. */
 ...
 *  ROM, not in the application, so there is nothing here to read.  This is
 *  the documented Toolbox generator: the minimal-standard Lehmer PRNG seed
 *  = seed * 16807 mod (2^31 - 1), returning the low 16 bits as a signed
 *  word.  Any claim about reproducing an exact sequence of the game's dice
 *  rolls inherits this assumption. */
int16_t Random(void)
{
