 ...
 *  prints through dumpf, and nothing else does.  The sink is stdout,
 *  where the tools in tools/ read the dumps.  The file --dump-to names,
 *  so a dump can be kept without the game's log in it.  The mesh check's
 *  own result lines are not dumps: they are the program's answer and
 *  stay on stdout. */
#ifndef ARC_DUMP_H
#define ARC_DUMP_H
#ifdef __cplusplus
