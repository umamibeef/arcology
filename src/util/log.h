 ...
 *  -> spdlog decides, which covers isatty, TERM and the Windows console A
 *  refusal beats a request, a request beats a guess.  Levels come from
 *  SPDLOG_LEVEL, which spdlog reads itself: SPDLOG_LEVEL=debug
 *  everything SPDLOG_LEVEL=warn               warnings and errors only
...
#define R_ERR(src, ...)  log_msg(R_LOG_ERROR, (src), __VA_ARGS__)
#define R_WARN(src, ...) log_msg(R_LOG_WARN, (src), __VA_ARGS__)
