                     ...
                     const char *out)
{
    ...
    dumpf("wrote %s  (shape %d, %ux%u, footprint %ux%u, rises %d px)\n", out,
...
int atlas_main(int argc, char **argv)
{
    ...
    dumpf("\nscale -> art set\n");
    {
        ...
        {
            const RAtlasLevel *l = atlas_level_for_scale(&a, probe[k]);
            dumpf("  %.2fx -> %d px%s\n", (double) probe[k],
                   l ? (int) l->zoom : 0,
