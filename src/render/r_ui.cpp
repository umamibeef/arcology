/*  r_ui.cpp -- the canon interface, hosted by Dear ImGui.
 *
...
extern "C" int r_ui_set_theme(RUi *u, const char *dir)
{
    if (!u)
        return -1;
    char path[1024];
...
static void palette(RUi *u, RUiState *s)
{
    ...
    for (int k = 0; k < N_PALETTE; ++k)
    {
        ...
        if (b.tool < 0)
        {
            ...
            r_ui_log(s, "Shift-click help: not yet ported");
            continue;
        }
        if (u->help_mode)
        {
            r_ui_log(s, "%s", RUI_TOOL_NAME[b.tool]);
            u->help_mode = false;
            continue;
        }
        switch (b.tool)
        {
            ...
            case RUI_TOOL_NEIGHBORS:
                r_ui_log(s, "Neighbors window: not yet ported");
                break;
            case RUI_TOOL_GRAPHS:
                s->show_graphs = 1;
                break;
            case RUI_TOOL_INDUSTRY:
                r_ui_log(s, "Industry window: not yet ported");
                break;
            ...
            case RUI_TOOL_LAYER_ZONES:
                r_ui_log(s, "%s layer toggle: not yet ported", RUI_TOOL_NAME[b.tool]);
                break;
            default:
...
static void menu_bar(RUi *u, RUiState *s)
{
    ...
    if (ImGui::BeginMenu("View"))
    {
        ...
        if (ImGui::BeginMenu("Pixel scale"))
        {
            for (int k = 1; k <= 4; ++k)
            {
                char lab[8];
...
extern "C" RUi *r_ui_create(SDL_Window *win, SDL_GPUDevice *dev, int swap_fmt, float dpi, const char *assets_dir)
{
    RUi *u = new RUi();
    memset(u, 0, sizeof *u);
    u->win = win;
...
extern "C" void r_ui_destroy(RUi *u)
{
    if (!u)
        return;
...
extern "C" int r_ui_event(RUi *u, const SDL_Event *e)
{
    if (!u)
        return 0;
...
extern "C" int r_ui_wants_mouse(const RUi *u)
{
    return u && ImGui::GetIO().WantCaptureMouse ? 1 : 0;
}

extern "C" int r_ui_wants_keyboard(const RUi *u)
{
    return u && ImGui::GetIO().WantTextInput ? 1 : 0;
}

extern "C" void r_ui_frame(RUi *u, RUiState *s)
{
    if (!u)
        return;
...
extern "C" void r_ui_render(void *ud, SDL_GPUCommandBuffer *cmd, SDL_GPUTexture *target, uint32_t w, uint32_t h)
{
    RUi *u = (RUi *)ud;
    (void)w;
    (void)h;
...
extern "C" void r_ui_log(RUiState *s, const char *fmt, ...)
{
    va_list ap;
