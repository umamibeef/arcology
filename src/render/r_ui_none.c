/*  r_ui_none.c -- the UI interface with no UI behind it. */
#include <stdarg.h>
#include <stdio.h>
#include <string.h>
...
RUi *r_ui_create(struct SDL_Window *win, struct SDL_GPUDevice *dev,
                 int swap_fmt, float dpi, const char *assets_dir)
{
    (void) win; (void) dev; (void) swap_fmt; (void) dpi; (void) assets_dir;
    return NULL;
}
void r_ui_destroy(RUi *u) { (void) u; }
int  r_ui_set_theme(RUi *u, const char *dir) { (void) u; (void) dir; return -1; }
int  r_ui_event(RUi *u, const union SDL_Event *e) { (void) u; (void) e; return 0; }
int  r_ui_wants_mouse(const RUi *u) { (void) u; return 0; }
int  r_ui_wants_keyboard(const RUi *u) { (void) u; return 0; }
void r_ui_frame(RUi *u, RUiState *s) { (void) u; (void) s; }
void r_ui_render(void *u, struct SDL_GPUCommandBuffer *cmd,
