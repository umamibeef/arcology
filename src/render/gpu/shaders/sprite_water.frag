 ...
 *  Gerstner waves.  With both flags off this is sprite.frag: the sprite as
 *  it is.  With the shader on, every pixel whose palette entry is water --
 *  the palette texture's alpha says so -- is shaded as a water surface
 *  instead; with the mesh on, the pixels that are not water, the sand rim,
 *  are dropped so the mesh ground runs up to the water.  The shading: two
 *  drifting layers of value noise perturb the normal, a highlight follows
 *  the sun, a small Fresnel term brings in the sky, all in world
 *  coordinates recovered from the tile's diamond, so the surface runs
 *  continuously from tile to tile like the ocean.  The base colour is the
 *  palette's own blue, so the original's tone survives. */
...
/*  One Gerstner wave, as the classic formulation has it: wave number k = 2
 *  pi / L, phase speed c = sqrt(g / k), phase f = k (d. p) - c t, amplitude
 *  a = S / k where S is the steepness.  The surface point moves by (d a cos
 *  f, a sin f) and the tangent and binormal are the derivatives of that, so
 *  the normal is analytic.  Only the normal and the height are used: the
 *  water is a decal on a flat tile and is not displaced. */
...
void main()
{
     ...
     *  its banks, so the distance adds a little depth of its own.  Tile
     *  (col, row) spans p in [col, col + 1); its texel's centre is
