...
void main()
{
    /*  The pass this draw is (gpu_frame.c): 0 the base, everything but a
     *  road's sidewalk zone and its paint; 1 the sidewalk pass (walk.c),
     *  a road strip's outer fifth alone; 2 the marking pass (marking.c),
     *  the lines alone, over the base at equal depth.  The same vertices
     *  each time: a pass is a draw, not geometry. */
    int  pass    = int(fr.flags.x + 0.5);
    bool roadmat = (v_col.b > 6.5 && v_col.b < 7.5) || (v_col.b > 9.5 && v_col.b < 10.5) || (v_col.b > 16.5 && v_col.b < 17.5);
    bool deckmat = v_col.b > 18.5 && v_col.b < 19.5;
    if (pass == 1 && !roadmat)
        discard;
    if (pass == 2 && !roadmat && !deckmat)
        discard;
    /* the junction's sidewalk band, built in: hidden while the sidewalk pass is off */
    if (v_col.b > 12.5 && v_col.b < 13.5 && v_col.r > 0.5 && fr.flags.y < 0.5)
        discard;
    ...
    if (v_col.b > 12.5 && v_col.b < 13.5)
    {
        ...
        if (fj.x < jw.x * 2.0 || fj.y < jw.y * 2.0)
            c *= 0.84;
        /*  A junction's sidewalk band carries the road's across in v_col.r,
         *  0.80 at its inner edge to 1.0 at the outline, and gets the
         *  road's curb along that inner edge, never under a pixel wide; a
         *  paved tile passes a constant and gets no curb. */
        if (v_col.r > 0.5 && v_col.r < 0.80 + clamp(fwidth(v_col.r), 0.05, 0.10))
            c = vec3(0.36, 0.36, 0.37);
    ...
    if (v_col.b > 17.5 && v_col.b < 18.5)
    {
         ...
         *  col.g set means the girder's face, which lies in the deck's own
         *  shadow. */
    ...
    {
        /*  A railway sized against the road as the road is sized to the
         *  tile (spec 5.9's order, bottom up), to spec 6.1's dimensions at
         *  fifteen metres to the tile: track centres 4.0 m apart, gauge
         *  1435 mm, ties 2.6 m long at 600 mm.  Ballast across the band,
         *  ties over it, and TWO rails to a track -- never four lines --
         *  the rails being the light element, their polished heads seen
         *  from above on mid-brown ties over light ballast, so that they
         *  stay apart at the base zoom.  Each rail is a device pixel at
         *  least and never wider than a tenth of the band.  The offsets
         *  below are fractions of the half band. */
        float x    = abs(v_col.r);
        float u    = abs(x - 0.43);
        float px   = fwidth(v_col.r);
        float rw   = clamp(max(px * 0.55, 0.04), 0.04, 0.08);
    ...
    if (v_col.b > 20.25)
    {
        /*  A show-curves ground highlight: the corridor's tan, the free
         *  air's blue, blended over the tile's own top under the roads. */
        int pn = int(v_col.r + 0.5);
        if (pn >= 8)
        {
            /*  The network's tiles, in the MAP VIEW only (params.w is -1
             *  there): a ramp orange, a road grey, a rail yellow, a highway
             *  purple ("roads highlighted in map view with some color (grey
             *  maybe), rails with yellow, and highways with purple"). */
            if (fr.params.w > -0.5)
                discard;
            o_col = pn == 8    ? vec4(0.98, 0.50, 0.08, 0.55)
                    : pn == 9  ? vec4(0.60, 0.60, 0.66, 0.45)
                    : pn == 10 ? vec4(0.95, 0.85, 0.20, 0.45)
                               : vec4(0.62, 0.30, 0.88, 0.45);
            return;
        }
        vec3 c = pn == 6 ? vec3(0.30, 0.50, 0.95) : vec3(0.92, 0.72, 0.28);
        o_col  = vec4(c, 0.38);
        return;
    }
    if (v_col.b > 19.5)
    {
         ...
         *  tint is a lid on the tile, so it has to carry the tile's edges
         *  or the map loses its grid exactly where it is most read. */
        if (fr.params.z > 0.0)
        {
            vec2  f    = fract(v_wpos.xy);
            vec2  edge = min(f, 1.0 - f);
    ...
    if (v_col.b > 18.5)
    {
        ...
        /*  The along coordinate carries the lane drop (spec 7.3) in its
         *  thousands: +1000 the negative side narrowed to two lanes, +2000
         *  the positive, +4000 a ramp lane on its own, 0.70 to 1.0 across.
         *  Whole thousands leave the dashes' phase alone. */
        float gg    = v_col.g;
        int   kn    = int(floor(gg / 1000.0 + 0.001));
        bool  strip = kn >= 4;
        bool  nl    = (kn - (strip ? 4 : 0)) >= 2;
        bool  nr    = (kn - (strip ? 4 : 0) - (nl ? 2 : 0)) >= 1;
        bool  narrow = (v_col.r < 0.0 && nr) || (v_col.r > 0.0 && nl);
        float lit  = (fr.sun.w + fr.params.x * d) /
                    max(fr.sun.w + fr.params.x * max(normalize(fr.sun.xyz).z, 0.0), 1e-3);
        lit        = 1.0 + 0.35 * (lit - 1.0);
        /*  Flat asphalt, no grain: the markings are markings, the surface
         *  is one colour. */
        vec3 c     = vec3(143.0, 143.0, 143.0) / 255.0;
        bool dash  = fract(gg * 4.0) < 0.5;
        if (strip)
        {
            /*  The ramp lane: an edge line either side, solid. */
            if ((x > 0.70 && x < 0.70 + line) || (x > 0.94 - line && x < 0.94))
                c = vec3(0.92);
        }
        else if (x < 0.11)
            ...
            c = vec3(0.92);
        else if (narrow && x > 0.70 - line && x < 0.70)
            c = vec3(0.92); /* two lanes on this side: the edge line where the third lane was */
        else if (!narrow && abs(x - 0.70) < 0.5 * line && dash)
            c = vec3(0.92);
        else if (x > 0.94 - line && x < 0.94)
            c = vec3(0.92);
        if (pass == 2 && c != vec3(0.92))
            discard; /* the marking pass on a deck: the lines alone */
        if (pass != 2 && c == vec3(0.92))
            c = vec3(143.0, 143.0, 143.0) / 255.0; /* the deck beneath: no line */
        o_col = vec4(clamp(c * lit, 0.0, 1.0), v_col.a);
        return;
    }
    if (v_col.b > 6.5 && v_col.b < 7.5 || v_col.b > 9.5 && v_col.b < 10.5 || (v_col.b > 16.5 && v_col.b < 17.5))
    {
         ...
         *  0x91) to 0.78, a curb, then a sidewalk of concrete with a joint
         *  every half tile.  Down the crown, by class in v_curv: a road has
         *  the sprites' light dashes (0x8B), nine to the tile; an avenue a
         *  double yellow line with a dashed lane line each side; a
         *  boulevard a planted median between curbs.  Every line is the
         *  art's width, one pixel at the base zoom, and never thinner than
         *  a device pixel. v_col.g runs along the strip in tiles; a
         *  negative v_col.g is a junction, no dashes.  The crosswalk is
         *  white bars across the asphalt, six to the width, with the solid
         *  stop line at its outer end. */
        float x       = abs(v_col.r);
        float px      = fwidth(v_col.r);
        ...
        vec3  c       = asphalt;
        /*  The road-marking pass (marking.c): every painted line below --
         *  dashes, centre lines, edge lines, crosswalk bars, stop lines,
         *  the crossing's X -- is drawn only while the frame says the
         *  pass is on; curb, sidewalk, median and barrier are the road
         *  itself and stay.  Off, the road is blank asphalt. */
        ...
        float sk      = 0.80;
        /*  The outer fifth is the sidewalk pass's: painted on pass 1 alone,
         *  discarded on every other; and pass 1 paints nothing else. */
        if (pass == 1 ? x <= sk : x > sk)
            discard;
        if (x > sk)
        {
        ...
        else if (cls > 2.5)
        {
             ...
             *  three lanes each way.  Across the half deck, in its own 14.9
             *  m: half the median barrier 0.3, inner shoulder 0.6, three
             *  3.7 m lanes, a 2.4 m outer shoulder and the 0.5 m parapet.
             *  Normalised, that is every number below.  Yellow on the LEFT
             *  against the median and white on the right is the freeway cue
             *  -- it is the pair that says at a glance which way the
             *  traffic runs. */
            if (x < 0.020)
        ...
        {
            bool painted = c == dash || c == yellow || c == vec3(0.92);
            if (pass == 2 && !painted)
                discard; /* the marking pass is the paint alone */
            if (pass == 0 && painted)
                c = asphalt; /* the base draw carries no paint */
        }
     ...
     *  fine grain on top. */
    vec2  p      = v_wpos.xy;
    /*  The ground reads its own topology.  Slope from the face, aspect from
     *  the face against the sun, altitude from the vertex, curvature from
     *  the height field, and the distance to water from the field texture:
     *  lush grass low, damp and in hollows; drier grass on ridges, sun-
     *  facing slopes and with altitude; earth and then rock as the slope
     *  steepens; sand on level ground at the waterline; a fine two-octave
     *  grain over all of it.  The material reads the SMOOTHED surface, the
     *  height field's gradient carried per vertex, so it grades across the
     *  facets instead of stepping at each one; the flat face normal lights
     *  it. */
