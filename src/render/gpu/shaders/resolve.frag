#version 450
/*  resolve.frag -- the frame target onto the window.  Nearest-neighbour by
 *  an integer pixel scale, so pixel art stays pixel art on a high-density
 *  display.  Outside the target the sky shows.  The shadow mask is applied
 *  here, by $19B76's rule: only a destination in the dirt ramp darkens --
 *  79 becomes 84, 100..110 become 110 -- and the index is read back out of
 *  the alpha channel the sprite pass wrote.  A lit surface from the mesh
 *  path carries no palette colour, so it is darkened multiplicatively
 *  instead. */
