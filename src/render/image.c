/*  image.c: the finished picture, as a file.  Freeing it, hashing it,
 *  and the four PNG writers.  They are the plain one, and the two that
 *  keep the GAME's palette indices rather than letting lodepng renumber
 *  them, and the provenance one. */
#include "soft/soft.h"
#include "project.h"
...
/*  Write the picture as a palette PNG carrying the GAME's palette
 *  indices.  That is not the same as letting lodepng fold the RGB back
 *  to a palette.  auto_convert builds its own table.  It uses the colors
 *  it finds, and present, and renumbers everything.  This is fine to
 *  look at and useless for anything that cares which index a pixel is.
 *  SC2K's animation is a rotation of indices 155..203 and 224..238.  So
 *  an export that renumbers them turns the animation into a no-op: which
...
int image_write_png_provenance(const RImage *im, const char *path)
{
     ...
     *  here is a picture.  It is a plane of ids that a checker reads
     *  back. */
    LodePNGState   st;
...
int image_write_png(const RImage *im, const char *path)
{
     ...
     *  uses few colors and writes a palette PNG: correct and smaller,
