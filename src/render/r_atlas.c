/*  r_atlas.c -- load the PNG atlases and JSON sidecars sc2kpack.py emits.
 *
 *  See r_atlas.h for the portability rules.  The short version: fixed-width
 ...
 *  palette resolve
 * ------------------------------------------------------------------ */
void r_atlas_resolve_rect(RAtlas *a, RAtlasLevel *l, int32_t x0, int32_t y0,
                          int32_t w, int32_t h)
{
    int32_t x, y;
    if (!l->indices || !l->rgba)
...
{
    r_atlas_resolve_rect(a, l, 0, 0, l->w, l->h);
}
                      ...
                      RAtlasLevel *l)
{
    ...
    /*  Keep phase 0 so r_atlas_animate can compose every later phase from
