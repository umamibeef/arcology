...
/*  The 2.5D passes. g_pass is 0 for the original's single sweep.  It is
 *  1 while the terrain is painted, which writes depth.  It is 2 while
 *  everything else is painted.  Everything else is tested against it.
 *  g_order holds the index of the tile that emitted the op being
 *  painted: strictly increasing along the original's sweep.  So "depth"
 *  here means "drawn later in the sweep", which is the only depth the
 *  original ever had. */
...
static void paint_shadow(RImage *im, const RAtlas *a, const RAtlasLevel *l, const ROp *op)
{
    ...
    for (yy = 0; yy < (int32_t)t->h; ++yy)
    {
        ...
        for (xx = 0; xx < (int32_t)t->w; ++xx)
        {
             ...
             *  dirt spur is considered at all, and only then does
...
int soft_render(RImage *out, const RAtlas *a, const RCity *c, const RSoftOpts *o)
{
    ...
    }
    {
         ...
         *  ($15452 _FillRect), not with the sky color. */
         ...
         *  palette entry like everything else.  Snap it to the nearest
         *  one and paint BOTH planes from that: otherwise an indexed
         *  export disagrees with the RGB one.  The shadow pass.  Which
         *  reads the index plane.  Would be testing a number no pixel
         *  has. */
        int  pi, best = 0;
        long bestd = -1;
        for (pi = 0; pi < 256; ++pi)
    ...
    if (dump)
    {
        /*  Report the top-left the game itself passes to $18E96.  It is
         *  in the game's own origin.  render_diff.py compares the anchor
     ...
     *  tile, so it has no terrain pass to split off.  It keeps the
     *  single sweep. */
...
/*  The depth plane of the last --mesh render, as a 16-bit gray PNG.  It
 *  holds the tile owning each pixel.  0 where no terrain was drawn.  A
 *  diagnostic, not a picture. */
