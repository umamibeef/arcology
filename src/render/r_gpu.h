 ...
 *  for pixel, and r_gpu_readback exists so that claim can be measured.
 *
 *  Two things can replace art with shading.  The land tiles' terrain art
 *  can give way to a mesh that replicates the sprites' geometry
 *  (r_gpu_set_mesh, RGpuView.terrain3d), drawn in the depth pass where
 *  the art was so everything else composes unchanged.  And the water and
 *  shore sprites can keep their outline while a water shader paints their
 *  blue pixels (RGpuView.water3d).  The two are independent.
 */
#ifndef R_GPU_H
#define R_GPU_H
...
typedef struct
{
    ...
    int terrain3d;              /* the mesh instead of the land art          */
    int water3d;                /* the water shader inside the water sprites */
    int roads3d;                /* the road strips on the mesh, in place of the
                                 * road sprites (with the mesh on)            */
...
RGpu *r_gpu_create(struct SDL_Window *win, const RAtlas *a, char *err, size_t err_len);
void  r_gpu_destroy(RGpu *g);

/*  The palette as it stands now (after r_atlas_animate); uploaded on the
...
int r_gpu_set_mesh(RGpu *g, const RMeshVert *v, uint32_t n, const RMeshVert *w, uint32_t nw); /* w: blended faces */
