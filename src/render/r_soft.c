/*  r_soft.c -- the reference rasteriser.  See r_soft.h.
 *
...
void r_soft_defaults(RSoftOpts *o)
{
    memset(o, 0, sizeof *o);
    o->zoom           = 32;
    o->n              = R_MAP;
 ...
 *
 *  ALTM therefore carries two heights.  Across the corpus the water field
 ...
 *  back through r_soft_focus_result. */
...
uint32_t r_soft_depth_blocked(void)
{
    return g_blocked;
}
...
{
    const RTile *t = r_atlas_tile(e->l, shap);
    ROp         *op;
                     ...
                     int32_t road_y, int road_flip)
{
    ...
    if (op)
    {
        const RTile *rt = r_atlas_tile(e->l, road);
                       ...
                       int32_t sy)
{
    ...
    {
        const RTile *t = r_atlas_tile(l, l->id_base + shape);
        if (t)
...
int r_sweep(const RAtlas *a, const RCity *c, const RSoftOpts *o, ROpList *ops,
            RSweep *info)
{
    ...
    mask       = r_city_corner_mask(c->rotation);
    g_focus_ok = 0;
    ...
    for (s = 0; s < 2 * n; ++s)
    {
        ...
        for (dy = 0; dy < n; ++dy)
        {
            ...
            sy = oy + (dx + dy) * (th / 2) -
                 (o->underground ? (int32_t) (c->altm[idx] & 0x1Fu)
                ...
                draws_here(c->xzon[idx], (uint8_t) b, mask))
            {
                ...
                const RTile *bt = r_atlas_tile(l, l->id_base + b);
                int32_t      drop, road_y;
                int          flip;
...
int r_soft_render(RImage *out, const RAtlas *a, const RCity *c,
                  const RSoftOpts *o)
{
...
int r_soft_write_depth_png(const RImage *im, const char *path)
{
    LodePNGState   st;
...
uint32_t r_image_crc(const RImage *im)
{
...
int r_soft_focus_result(int32_t *x, int32_t *y)
{
    if (!g_focus_ok)
        return 0;
    *x = g_focus_x;
 ...
 */
int r_image_write_png_indices(const RImage *im, const RAtlas *a,
                              const char *path)
{
    LodePNGState   st;
...
int r_image_write_png_indexed(const RImage *im, const char *path)
{
    LodePNGState   st;
    unsigned char *png = NULL;
    size_t         n   = 0;
...
int r_image_write_png_provenance(const RImage *im, const char *path)
{
...
int r_image_write_png(const RImage *im, const char *path)
{
