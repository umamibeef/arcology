...
{
    const RShaderSrc       *s = r_shader_find(name);
...
RGpu *r_gpu_create(SDL_Window *win, const RAtlas *a, char *err, size_t err_len)
{
    ...
    r_gpu_set_light(g, 0.35f, 0.85f, 0.9f, 0.5f, 0.6f);
    return g;
}

void r_gpu_destroy(RGpu *g)
{
    int32_t i;
    if (!g)
        return;
...
void r_gpu_set_palette(RGpu *g, const RAtlas *a)
{
    int i;
    for (i = 0; i < 256; ++i)
    {
...
int r_gpu_set_shore(RGpu *g, const uint8_t *field, int32_t n)
{
    if (n != R_MAP)
        return -1;
...
void r_gpu_set_light(RGpu *g, float x, float y, float z, float ambient, float diffuse)
{
    g->sun[0]    = x;
    g->sun[1]    = y;
    g->sun[2]    = z;
...
int r_gpu_set_ops(RGpu *g, const ROpList *ops, const RSweep *sw)
{
    ...
    for (k = 0; k < ops->n; ++k)
    {
        ...
        const RTile *t  = r_atlas_tile(l, op->shape);
        RInst       *in;
        if (!t)
            continue;
        in         = &g->inst[g->n_inst];
        ...
        in->misc[1] = in->misc[2] = in->misc[3] = 0.0f;
        ...
        {
            const RTile *ut = r_atlas_tile(l, op->under_shape);
            if (ut)
            {
...
int r_gpu_set_mesh(RGpu *g, const RMeshVert *v, uint32_t n, const RMeshVert *w, uint32_t nw)
{
    uint32_t n_opaque = n;
    uint32_t bytes;
    n += nw;
...
int r_gpu_set_movers(RGpu *g, const RMeshVert *v, uint32_t n)
{
...
typedef struct
{
    ...
    float rot[4];  /* rotating (1 or 0), pivot column, pivot row, 0          */
} CamU;
...
static int build_visible(RGpu *g, const RGpuView *v, Ranges *r)
{
    ...
    for (pass = 0; pass < 4; ++pass)
    {
        ...
        for (k = 0; k < g->n_inst; ++k)
        {
            ...
            if (pass == 0)
                want = (kind == K_TERRAIN) ||
                       (kind == K_LAND_ART && !v->terrain3d) ||
                       (kind == K_UG_LATTICE && !(v->terrain3d && v->underground));
            else if (pass == 1)
                want = (kind == K_WATER_ART || kind == K_WATER_COL ||
                        kind == K_WATER_EDGE) &&
                       !v->terrain3d; /* meshed, the water is a body of faces */
            else if (pass == 2)
                want = (kind == K_SPRITE) ||
                       ((kind == K_ROAD_ART || kind == K_CAR || kind == K_TRAIN) && !(v->terrain3d && v->roads3d));
            else
                want = (kind == K_SHADOW) && v->angle == 0.0f;
            if (!want)
                continue;
            if (v->angle == 0.0f &&
...
static void cam_for(const RGpu *g, const RGpuView *v, CamU *u)
{
    ...
    u->rot[0]  = v->angle != 0.0f ? 1.0f : 0.0f;
    u->rot[1]  = v->pivot_c;
    u->rot[2]  = v->pivot_r;
    u->rot[3]  = 0.0f;
}
...
static void draw_mesh(RGpu *g, SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *rp, const RGpuView *v, const CamU *cam, const RAtlasLevel *l)
{
    ...
    if (v->terrain3d && g->mesh_n && g->pipe_mesh)
    {
        LightU lu;
        (void)l;
...
static int draw_frame(RGpu *g, SDL_GPUCommandBuffer *cmd, const RGpuView *v, SDL_GPUTexture *final, SDL_GPUGraphicsPipeline *resolve_pipe, int32_t sw, int32_t sh, float scale)
{
    ...
    if (build_visible(g, v, &r) != 0)
        return -1;
    if (v->mesh_only)
        r.n_terrain = r.n_water = r.n_sprite = r.n_shadow = 0;
    ...
    fu.p[1]   = v->terrain3d ? 1 : 0; /* the water sprites drop their rim  */
    fu.p[2]   = v->water3d ? 1 : 0;   /* and shade their water            */
    fu.f[0]   = v->time;
    ...
    if (getenv("SC2K_GPU_DEBUG"))
        R_DBG("gpu", "terrain %u water %u sprite %u shadow %u, mesh %u", (unsigned)r.n_terrain, (unsigned)r.n_water, (unsigned)r.n_sprite, (unsigned)r.n_shadow, (unsigned)(v->terrain3d ? g->mesh_n : 0));
...
int r_gpu_frame(RGpu *g, const RGpuView *v, const uint8_t sky[3], RGpuOverlay overlay, void *ud)
{
    SDL_GPUCommandBuffer *cmd;
...
int r_gpu_readback(RGpu *g, const RGpuView *v, const uint8_t sky[3], int32_t w, int32_t h, int32_t scale, RImage *out, RGpuOverlay overlay, void *ud)
{
    SDL_GPUCommandBuffer      *cmd;
    SDL_GPUCopyPass           *cp;
    SDL_GPUFence              *fence;
...
struct SDL_GPUDevice *r_gpu_device(const RGpu *g)
{
    return g->dev;
}

int r_gpu_swapchain_format(const RGpu *g)
{
    return (int)g->swap_fmt;
}

int r_gpu_offscreen_format(const RGpu *g)
{
    return (int)g->color_fmt;
}

const char *r_gpu_driver(const RGpu *g)
{
    const char *d = SDL_GetGPUDeviceDriver(g->dev);
    return d ? d : "?";
}

void r_gpu_stats(const RGpu *g, uint32_t *instances, uint32_t *culled)
{
