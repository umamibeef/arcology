 ...
 *  stands at, so what it answers for one junction is not what it answers
 *  for another.
 *
 *  Nothing here ships a shape and nothing here holds a number.  Take the
 *  scripts away and there are no props at all.  What C keeps is the
 *  walk that turns the pieces into faces, and a cache of the last size
 *  each model was asked for, so a prop drawn every frame is built once.
 */
...
typedef enum
{
    ...
    M_PRISM    /* a box along the facing, cut on the tile folds: `d` its length,
                *  `ac` and `ac2` how far its near and far ends' feet stand
                *  over the surface the prop is put on */
} ModelKind;
...
typedef struct ModelPart
{
     ...
     *  from the pole to the head, and both are numbers of their own. */
    float ax, ac, ac2, w, d, z0, z1;
    /*  Where each foot of a prism stands along the run between the two
     *  heights it is put on: 0 at the near one, 1 at the far.  A car's
     *  body spans the whole run and its cabin the middle of it. */
    float f0, f1;
    float mat, code;
} ModelPart;
 ...
 *  its feet go, and where its own origin is from the point it was put
 *  at -- along the way it faces and across it.  A signal is put at the
 *  tile's middle and stands itself out at the kerb, so where it goes is
 *  the model's business and not the caller's. */
...
/*  The pieces one model is made of at that size, built by the script
 *  that defines it and kept until it is asked for another size or the
 *  scripts are read again.  Answers how many pieces there are. */
 ...
 *  a lamp beside a strip stands on the strip's own height, which the
 *  ground under the kerb is not. */
