/*  mesh_tile.c -- what a tile is, and the ground field.
 *
...
float node_altitude(const RCity *c, int32_t col, int32_t row)
{
     ...
     *  global to solve (the user: "I don't like global solves because that
     *  doesn't scale.  I think the right approach is to use the altitude
     *  of intersections and then ramp the corridors smoothly from
     *  those"). */
...
Kind tile_top(const RCity *c, int32_t col, int32_t row, uint8_t mask_bit, float z[4])
{
    ...
    /*  A corridor's tile IS the corridor's surface (the user: "this
     *  surface defines the top of the column of the tiles that make up
     *  the corridor"): its four corners were graded in the first pass,
     *  so it draws the field like open land and the corridor comes out
     *  as one continuous shelf.  A network tile is a structure, and a
     *  structure used to draw a flat pad at its own altitude, which left
     *  the shelf in pieces and the road standing over them. */
    /*  A corridor tile stands on the corridor's own surface, its own
     *  four corners of it.  The terrain field is never touched: a corner
     *  there is shared with the tile next door, so notching through it
     *  would drag the ground outside the corridor down with it (the user:
     *  "the terrain should remain exactly where it is").  The step
     *  between the notch and the ground it was cut from is closed by the
     *  wall rule, which is what a notch looks like. */
    if (is_structure(xbld) && xbld < 0x49u && s_pass == 2 &&
    ...
    if (is_structure(xbld) && xbld < 0x49u && s_pass == 2 && s_corr[row * GRID + col] == 1 &&
        ...
        for (col = 0; col < R_MAP; ++col)
        {
            ...
            for (k = 0; k < 4; ++k)
            {
                ...
                else if (structure)
                {
                     ...
                     *  whole difference. */
                    /*  Its base, not its plane: a corner every tile
                     *  around is a structure falls back to this average,
                     *  and a ramp's lifted plane put it a level over the
                     *  land beside it, a saddle in the field drawn as a
                     *  pair of twisted brown wedges at the foot of every
                     *  elevated highway (Oakland, column 104, row 44). */
