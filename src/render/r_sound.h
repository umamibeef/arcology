/*  r_sound.h -- the game's sounds, played through SDL's audio.
 *
...
RSound *r_sound_create(const char *assets_dir);
void    r_sound_destroy(RSound *s);
void    r_sound_play(RSound *s, int id);
int     r_sound_loaded(const RSound *s);
