/*  walk.c: THE NETWORK WALK.  It runs from a node to the next node,
 *  along the pieces a family lays.  It holds a segment's stages.  The
 *  cells, the class, the fit, the arms, the trims, the lanes and the
 *  overlay.  The pass over the map that drives them is build_networks.
 *
 *  What a family does at each stage is its own declaration's, in
 *  scripts/families, and not this file's.  The order the build steps
 *  through the network in is cursor.c's. */
#include <math.h>
#include <stdio.h>
#include <string.h>
#include <time.h>
...
static int seg_fit(Seg *x)
{
    ...
    int32_t      cc       = x->cc;
    int32_t      cr       = x->cr;
    /*  Stage one and two (mesh/fit.c): the corridor is the segment's own
        ...
        for (which = 0; which < 2; ++which)
        {
            ...
            ang = atan2f(dir.y, dir.x);
            {
                ...
                margin_add(MARGIN_CAP, c0, c1, (V2){0.0f, 0.0f}, (V2){0.0f, 0.0f});
                {
                    ...
                    w.row     = nr;
                    w.e       = ne;
                    w.w       = hw * geo_num(&gix_walk_cap_w, "walk_cap_w");
                    w.end[0]  = c0;
                    w.end[1]  = c1;
...
trims:
    ...
    net_prof_add(NET_PROF_OVERLAY, prof_now() - t2);
    {
        ...
        for (k = 0; k < x.nt && d.records_only; ++k)
            if (incr_want_tile(tcol[k], trow[k]))
                d.records_only = 0;
        x.records_only = d.records_only;
        for (k = 0; k < x.nt && !d.hot; ++k)
            if (incr_near(tcol[k], trow[k]))
                d.hot = 1;
