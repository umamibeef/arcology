 ...
 *  tile becomes a canvas pixel in four places: here for picking and the
 ...
 *  sprite actually lands.  The first inventory of this said three and left
 *  out the sweep -- the one place the answer is compared with the original
 *  pixel for pixel.  They were three separate transcriptions of the same
 *  arithmetic, and they have to agree exactly or the query box stops
 ...
 *  integer `tile_w / 2` in the other, which agree only because every tile
 *  width the game ships is even.  The shader cannot include this, so it
 *  takes the same numbers as uniforms, and tools/project_check.py (the
 *  `projection` test) reads the values below and fails if any C or GLSL
 *  restates one.  ARC_PITCH_SIN0 and ARC_PITCH_COS0 are the sine and cosine
 *  of the game's own camera pitch, 30 degrees.  Every other pitch is
 *  expressed as a ratio against them, so at 30 the ratios are 1 and the
 *  projection is the original's to the pixel. */
#ifndef R_PROJECT_H
#define R_PROJECT_H
 ...
 *  Below it the painter's slot still orders the tiles; above it, or turned,
 *  it cannot and the depth buffer takes over. */
