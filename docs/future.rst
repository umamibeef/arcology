...
.. container:: col
   ...
   **The canon interface is the original's, and its port has begun.** The game's interface (:doc:`enhanced-renderer`) is the original's tool palette from its own picture, its menus from its own menu resources, and System 7 chrome, with Dear ImGui as the engine underneath and behind a C interface (``src/render/r_ui.h``). What remains: the sub-tool menus and the tools themselves, which need the original's placement routines reconstructed; the map, ordinances, industry, neighbours and newspaper windows from their pictures; the shift-click help from ``TEXT`` 1100 onward; and rotation.
