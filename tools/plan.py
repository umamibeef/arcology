...
allows me to inspect a city?").
    ...
    tools/plan.py Bayview out.png 60 100 24 24 60 ... at 60 px a tile
    tools/plan.py - out.png                       a dump on stdin
    tools/plan.py Bayview out.png 60 100 24 24 60 -- --tune 0.5,0.62,0.9,3,6,8,0.8,0.04,0.45,0,0,0,0.6,0
                                                  anything after -- goes to the renderer:
                                                  that one draws the old fillet fit
...
def run(city):
    #  The path as given first, then the repository's own corpus, then the
    #  game folder.  The game folder came first once, and on a
    #  case-insensitive disk "cities/toronto.sc2" found its Cities/
    #  TORONTO.SC2 -- the Mac release, data in the resource fork, which
    #  the loader rejects -- and the tool reported no dump at all.
    path = city
    if not os.path.isfile(path):
        path = os.path.join(ROOT, "cities", city)
    if not os.path.isfile(path):
    ...
    out = subprocess.run(
        [os.path.join(ROOT, "build", "arcology"), os.path.join(ROOT, "assets"), path, "--mesh-check", "--roads3d",
         "--plan-dump", "--path-dump", "--junc-dump"] + EXTRA,
        capture_output=True,
        text=True,
    )
    return out.stdout
...
def parse(text):
    d = {"field": [], "xbld": [], "xter": [], "segs": [], "juncs": [], "spirals": []}
    it = iter(text.split("\n"))
    cur = None
    for line in it:
        ...
        elif line.startswith("TLIM"):
            cur["tlim"] = [float(v) for v in line.split()[1:]]
        elif line.startswith("PRIM"):
            t = line.split()
...
def fillet(pts, rad, tlim=None, steps=12):
    """The polyline with each corner replaced by its arc, as fillet_t
    builds it: a line to the tangent point, the arc, and on.  The tangent
    budget is the fit's own when the dump carries one, else half of each
    edge, which is what the older fits assumed."""
    import math
    if len(pts) < 3:
        return pts
    out = [pts[0]]
    ...
    for i in range(1, len(pts) - 1):
        ...
        theta = math.acos(dot)
        cross = ax * by - ay * bx
        lim = tlim[i] if tlim and i < len(tlim) else 0.5 * min(la, lb)
        lim = min(lim, math.hypot(pts[i][0] - cur[0], pts[i][1] - cur[1]), lb)
...
def main():
    ...
    args = sys.argv[1:]
    if "--" in args:
        EXTRA.extend(args[args.index("--") + 1:])
        args = args[: args.index("--")]
    city, out = args[0], args[1]
    ...
    n = len(d["xbld"])
    if len(args) >= 6:
        c0, r0, nc, nr = (int(v) for v in args[2:6])
    else:
        c0, r0, nc, nr = 0, 0, n, n
    scale = int(args[6]) if len(args) >= 7 else max(3, min(40, 1400 // max(nc, nr)))
    ...
    for s in segs:
        ...
        for kind, a, b in s.get("prims", []):
            cv.line(X(a[0]), Y(a[1]), X(b[0]), Y(b[1]), (80, 220, 230) if kind == 0 else (230, 90, 210),
                    3 if scale >= 8 else 1)
        pts = fillet(pts, s.get("rad", []), s.get("tlim") or None)
    ...
    import math as _m
    for cen, rad, a0, a1 in d["spirals"]:
        if not (c0 <= cen[0] < c0 + nc and r0 <= cen[1] < r0 + nr):
            continue
        steps = max(24, int(abs(a1 - a0) * 12))
        for k in range(steps):
            t0, t1 = a0 + (a1 - a0) * k / steps, a0 + (a1 - a0) * (k + 1) / steps
            cv.line(X(cen[0] + rad * _m.cos(t0)), Y(cen[1] + rad * _m.sin(t0)),
                    X(cen[0] + rad * _m.cos(t1)), Y(cen[1] + rad * _m.sin(t1)), (245, 220, 70), 2)
    for j in d["juncs"]:
