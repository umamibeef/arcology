...
#  Whole layers, not a list of files that grows every time one is split.
ALLOWED_DIRS = ("app/",)
ALLOWED = {"render/adapt.h", "render/adapt.c"}
...
def main():
    ...
    for path in sorted(SRC.rglob("*")):
        ...
        if rel.startswith(("sim/", "vendor/") + ALLOWED_DIRS) or rel in ALLOWED:
            continue
    ...
    if bad:
        ...
        for b in bad:
            print("   ", b)
        print("\nOnly src/app and %s may.  Code that authors a City belongs in src/sim."
              % ", ".join(sorted(ALLOWED)))
        return 1
    print("seam_check: src/app and %s see the simulation, as intended; %d other files name "
